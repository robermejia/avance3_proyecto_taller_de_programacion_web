...
function actualizarContador() {
    ...
    cont.textContent = carrito.length; // Actualiza con la cantidad de items en el carrito
}

// Modifica la función agregarAlCarrito para actualizar el contador
function agregarAlCarrito(nombre, precio) {
    carrito.push({ nombre, precio });
    mostrarMensajeFlotante(nombre + " ha sido agregado al carrito.");
    actualizarContador(); // Llama a actualizarContador después de agregar
}
...
function abrirCarrito() {
    ...
    carrito.forEach((item, index) => {
        ...
        btnEliminar.onclick = function() {
            carrito.splice(index, 1);
            abrirCarrito(); // Actualizar el carrito
            actualizarContador(); // Llama a actualizarContador después de eliminar
        };
        li.appendChild(btnEliminar);
        lista.appendChild(li);
        suma += item.precio;
...
function vaciarCarrito() {
    carrito = [];
    abrirCarrito();
    actualizarContador(); // Actualiza el contador al vaciar el carrito
}
